...
% [T, SM] = OTSUTHRESH(H) computers an optimum threshold, T, in the range
...
% bits). (i must be a column vector for processing below.)
i = (1:numel(h))';
...
% optimum threshold. There may be several contiguous max values.
% Average them to obtain the final threshold.
maxSigsq = max(sigSquared);
T = mean(find(sigSquared==maxSigsq));

% Normalized to range [0 1]. 1 is subtracted because MATLAB indexing starts
% at 1, but image intensities start at 0.
T = (T-1)/(numel(h)-1);
